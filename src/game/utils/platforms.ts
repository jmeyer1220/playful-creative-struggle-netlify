...
function createGridBackground(scene: Phaser.Scene) {
  ...
  // Draw vertical lines
  for (let x = 0; x < scene.cameras.main.width; x += GRID_SIZE) {
    graphics.moveTo(x, 0);
    graphics.lineTo(x, scene.cameras.main.height);
  }
  
  // Draw horizontal lines
  for (let y = 0; y < scene.cameras.main.height; y += GRID_SIZE) {
    graphics.moveTo(0, y);
    graphics.lineTo(scene.cameras.main.width, y);
  }
